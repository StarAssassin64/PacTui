...
func installPackages(packages []string) bool {
	var err error
	packageInstallLog, err = exec.Command("pacman", "-Ssq", strings.Join(packages, " ")).Output()
	if err != nil {
		return false
	}
	packageInstallLog, err = exec.Command("tail", "-40", packageInstallLog).Output()
	if err != nil {
		return false
	}
	return true
